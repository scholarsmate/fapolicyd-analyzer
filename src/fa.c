...
hash_set_t * get_deny_rules(rules_t * rules_ptr) {
    return rules_ptr->deny_rules;
}
...
int rule_create(char * buf, size_t buf_len,
    const char * dec, const char * perm, uint32_t uid, const char * exe, bool trust_subject,
    const char * path, bool trust_object) {
        const char * sub_trust = (trust_subject) ? " trust=1" : "";
        const char * obj_trust = (trust_object) ? " trust=1" : "";
        return snprintf(buf, buf_len - 1, "%s perm=%s auid=%u exe=%s%s : path=%s%s", dec, perm, uid, exe, sub_trust, path, obj_trust);
}
